...
lay[,2]<-(TrophInd(arctic.matrix)$TL-1)^4
...
lay[,2]<-(TrophInd(europe.matrix)$TL-1)^4
...
lay[,2]<-(TrophInd(serengeti.matrix)$TL-1)^4
lay[lay[,2]>5,2]<-rnorm(sum(lay[,2]>5), mean = 2, sd = 0.2)
...
lay[,2]<-scale(TrophInd(pyrenees.matrix)$TL-1) + rnorm(nrow(pyrenees.matrix), mean =0,sd = 0.2)
